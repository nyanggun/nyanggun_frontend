				...
				{!isEditing ? (
					<>
						<div className="mypage-info-list">
							<p>
								<strong>이메일</strong> {profileData.email || "등록된 이메일 없음"}
							</p>
							<p>
								<strong>닉네임</strong> {profileData.nickname || "사용자"}
							</p>
							<p>
								<strong>전화번호</strong> {profileData.phoneNumber || "등록된 번호 없음"}
							</p>
						</div>
						<div className="mypage-action-buttons">
							{user && user.role === "ROLE_ADMIN" && (
								<BorderButton btnName="유저 제재" clickBtn={() => {}} />
							)}
							...
							{user?.id === profileData?.id && (
								<BorderButton btnName="회원탈퇴" clickBtn={handleWithdrawalClick} buttonColor="red" />
							)}
						</div>
					</>
				) : (
					<>
						<div className="mypage-edit-form">
							...
							<div className="mypage-form-buttons d-flex gap-2 justify-content-start mt-3">
								<BorderButton btnName="저장" clickBtn={handleSave} buttonColor="green" />
								<BorderButton btnName="취소" clickBtn={() => setIsEditing(false)} buttonColor="gray" />
							</div>
						</div>
					</>
				)}
