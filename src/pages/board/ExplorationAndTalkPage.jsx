...
const ExplorationAndTalkPage = () => {
  ...
  const handleSearch = (keyword) => {
    if (location.pathname.startsWith("/dorandoran/explorations")) {
      ...
      navigate(
        `/dorandoran/explorations/search?keyword=${encodeURIComponent(keyword)}`
      );
      return;
      ...
      setSearchKeyword(keyword);
      navigate(
        `/dorandoran/talks/search?keyword=${encodeURIComponent(keyword)}`
      );
    }
  };
  return (
    <div>
      <div className="board-page">
        <div className="search-margin">
          <Subtitle text={"도란도란"} onSearchBoard={handleSearch}></Subtitle>
        </div>
      </div>
      ...
      {isPostListPage && userData.user !== null ? (
