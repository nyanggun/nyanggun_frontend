...
import HomeExploration from "../../components/board/HomeExploration";
...
const Home = () => {
  ...
  const handleExploration = async () => {
    try {
      const response = await api.get("/home/exploration");
      setExplorationList(response.data.data);

      console.log("문화재 탐방기를 북마크 순으로 가져왔습니다.", response.data);
    } catch (error) {
      console.log("문화재 탐방기를 가져오는 데 오류 발생");
    }
  };

  const goToExploration = (exploration) => {
    navigate(`/dorandoran/explorations/${exploration.id}`);
  };
          ...
          {explorationList && explorationList.length > 0 ? (
            explorationList.map((exploration, index) => (
              <HomeExploration
                ...
                onClick={() => goToExploration(exploration)}
              ></HomeExploration>
            ))
          ) : (
            <div>북마크가 되어있는 탐방기가 없습니다.</div>
          )}
        </div>
        <div className="item2">
          <div
