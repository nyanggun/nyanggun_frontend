...
const MyPagePhotoBoxBookmark = () => {
  ...
  const onRequestAppend = async () => {
    ...
    try {
      // console.log("커서 요청 2:");
      const response2 = await api.get(`/mypage/${userId.id}/photoboxbookmark`, {
  ...
  const handlePhotoBoxList = async () => {
    ...
    try {
      ...
      const response1 = await api.get(`/mypage/${userId.id}/photoboxbookmark`);
...
export default MyPagePhotoBoxBookmark;
